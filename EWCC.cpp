...
int weight[maxm];
int dscore[maxn], greedy_uncov_degs[maxn], confChange[maxn];
	...
	for (int v = 1; v <= N ; v++) {
		if (u!=v && adj[u][v]) {
			...
			if (!C[v]) {
				...
				L.push_front(e); // add to UL as the youngest node
				dscore[v] += weight[e];
				dscore[u] += weight[e];
			}
...
void remove_delta() {
	...
	if (v != 0) remove(v);
	if (debug) cout << "[Remove delta] " << v << endl;
}

void find_pairs(int v, vector<pair<int,int>> &S) {
	if (!confChange[v]) return;
	for (int u = 1; u <= N; u++) {
		if (u == v) continue;
		else if (adj[u][v]) {
			int e = adj[u][v];
			if (C[u] && dscore[u] + dscore[v] + weight[e] > 0 && u != lastremove) 
				S.push_back(make_pair(u, v));
		}
		else {
			if (C[u] && dscore[u] + dscore[v] > 0 && u != lastremove) 
				S.push_back(make_pair(u, v));
		}
	}
}
		...
		if (!L.empty()) {
			...
			if (u == 0 && v == 0) {
				...
				for (auto it = L.begin(); it != L.end(); it++) {
					int e = *it;
					if (--randEdge == 0) v = (randint(2) == 0 ? edges[e].x : edges[e].y);
					weight[e] += 1;
					confChange[edges[e].x] = 1;
					confChange[edges[e].y] = 1;
					dscore[edges[e].x] += 1;
					dscore[edges[e].y] += 1;
				}
				// randomly pick u
